...
async function main() {
  ...
  const userDefs = [
    { name: "Alice Chen",      email: "alice@fxau.io",    role: "admin",   plan: "premium" },
    { name: "Bob Smith",       email: "bob@fxau.io",      role: "admin",   plan: "premium" },
    { name: "Carol Rodriguez", email: "carol@fxau.io",    role: "mentor",  plan: "premium" },
    { name: "David Kim",       email: "david@fxau.io",    role: "mentor",  plan: "premium" },
    { name: "Emma Wilson",     email: "emma@fxau.io",     role: "trader",  plan: "basic"   },
    { name: "Frank Lee",       email: "frank@fxau.io",    role: "trader",  plan: "premium" },
    { name: "Grace Martinez",  email: "grace@fxau.io",    role: "trader",  plan: "basic"   },
    ...
    { name: "James Taylor",    email: "james@fxau.io",    role: "student", plan: "basic"   },
  ] as const
  ...
  for (const { name, email, role, plan } of userDefs) {
    const user = await prisma.user.create({
      data: {
        ...
        plan: plan as "none" | "basic" | "premium",
  ...
  await prisma.subscription.createMany({
    data: [
      { userId: emma.id,     plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(0),  status: "active",   verifiedAt: daysBack(25),  expiresAt: daysAhead(5)   },
      { userId: emma.id,     plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(1),  status: "expired",  verifiedAt: daysBack(60),  expiresAt: daysBack(30)   },
      { userId: emma.id,     plan: "premium", billing: "monthly", amountUSDT: 39,  network: "erc20", txHash: tx(2),  status: "rejected", verifiedAt: null,          expiresAt: null           },
      { userId: frank.id,    plan: "premium", billing: "yearly",  amountUSDT: 299, network: "erc20", txHash: tx(3),  status: "active",   verifiedAt: daysBack(90),  expiresAt: daysAhead(275) },
      { userId: frank.id,    plan: "premium", billing: "monthly", amountUSDT: 39,  network: "trc20", txHash: tx(4),  status: "expired",  verifiedAt: daysBack(180), expiresAt: daysBack(150)  },
      { userId: frank.id,    plan: "premium", billing: "yearly",  amountUSDT: 299, network: "trc20", txHash: tx(5),  status: "pending",  verifiedAt: null,          expiresAt: null           },
      { userId: grace.id,    plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(6),  status: "active",   verifiedAt: daysBack(10),  expiresAt: daysAhead(20)  },
      { userId: grace.id,    plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(7),  status: "expired",  verifiedAt: daysBack(210), expiresAt: daysBack(180)  },
      { userId: grace.id,    plan: "premium", billing: "monthly", amountUSDT: 39,  network: "erc20", txHash: tx(8),  status: "pending",  verifiedAt: null,          expiresAt: null           },
      { userId: henry.id,    plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(9),  status: "pending",  verifiedAt: null,          expiresAt: null           },
      { userId: henry.id,    plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(10), status: "expired",  verifiedAt: daysBack(95),  expiresAt: daysBack(65)   },
      { userId: henry.id,    plan: "premium", billing: "monthly", amountUSDT: 39,  network: "erc20", txHash: tx(11), status: "rejected", verifiedAt: null,          expiresAt: null           },
      { userId: isabella.id, plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(12), status: "pending",  verifiedAt: null,          expiresAt: null           },
      { userId: isabella.id, plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(13), status: "expired",  verifiedAt: daysBack(45),  expiresAt: daysBack(15)   },
      { userId: james.id,    plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "erc20", txHash: tx(14), status: "active",   verifiedAt: daysBack(5),   expiresAt: daysAhead(25)  },
      { userId: james.id,    plan: "basic",   billing: "monthly", amountUSDT: 29,  network: "trc20", txHash: tx(15), status: "expired",  verifiedAt: daysBack(65),  expiresAt: daysBack(35)   },
      { userId: carol.id,    plan: "premium", billing: "yearly",  amountUSDT: 299, network: "erc20", txHash: tx(16), status: "active",   verifiedAt: daysBack(120), expiresAt: daysAhead(245) },
      { userId: david.id,    plan: "premium", billing: "yearly",  amountUSDT: 299, network: "trc20", txHash: tx(17), status: "active",   verifiedAt: daysBack(200), expiresAt: daysAhead(165) },
      { userId: alice.id,    plan: "premium", billing: "yearly",  amountUSDT: 299, network: "erc20", txHash: tx(18), status: "active",   verifiedAt: daysBack(150), expiresAt: daysAhead(215) },
      { userId: bob.id,      plan: "premium", billing: "yearly",  amountUSDT: 299, network: "trc20", txHash: tx(19), status: "active",   verifiedAt: daysBack(100), expiresAt: daysAhead(265) },
    ],
  })
